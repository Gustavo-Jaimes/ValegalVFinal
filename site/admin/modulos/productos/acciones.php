...
//    Artículo Editar                   
	if(isset($_REQUEST['editar']) OR isset($editarNuevo)) {
		...
		foreach ($_POST as $key => $value) {
			if ($key=='txt' || $key == 'objetivo' || $key == 'dirigido' || $key == 'reserva' || $key=='especificaciones') {
...
// /%%%%%%%%%%%%%%%%%%%% Subir PDF %%%%%%%%%%%%%%%%%%%%/
	 if(isset($_REQUEST['pdf'])){
		...
		$i = strrpos($pdfName,'.');
		$l = strlen($pdfName) - $i;
		$ext = strtolower(substr($pdfName,$i+1,$l));
		$pdf= $CONEXION -> query("SELECT * FROM productos WHERE id = $id");
		$row_pdf = $pdf ->fetch_assoc();
		$rutaFinal='../img/contenido/'.$modulo.'/';
		if(file_exists($rutaFinal.$row_pdf['pdf'] && $row_pdf != "")){
			unlink($rutaFinal);
		}
		...
		if(file_exists($rutaInicial.$pdfName)){
			...
			$sql = "UPDATE $modulo SET pdf = '$imgFinal' WHERE id = $id";
			
			
			if($insertar = $CONEXION->query($sql)){
				copy($rutaInicial.$pdfName, $rutaFinal.$imgFinal);
				unlink($rutaInicial.$pdfName);
				$exito=1;
...
if(isset($_POST['borrarpdf'])){
	...
	if($borrar = $CONEXION->query("UPDATE $modulo SET pdf = '' WHERE id = $id")){
		$exito = true;
	}
	if(isset($exito)){
		echo "<div class='bg-success color-blanco'><i uk-icon='icon: trash;ratio:2;'></i> &nbsp; Borrado</div>";
...
if(isset($_REQUEST['imagen'])){
	...
	if(file_exists($rutaInicial.$imagenName)){
		...
		$sql = "INSERT INTO $modulopic (orden,imagen,producto) VALUES (99,'$imgFinal',$id)";
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%    Importar datos       %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	if(isset($_POST['importardatos'])){
		...
		foreach ($infoImportar as $key => $value) {
			$numTotal++;
			$prodSKU=trim($value[2]);
			$sql0="SELECT * FROM productos WHERE sku = '$prodSKU'";
			$CONSULTA = $CONEXION->query($sql0);
			$existe = $CONSULTA->num_rows;
			if ($existe==0 AND !isset($bandera)) {
				$bandera=1;
				$sql = 'INSERT INTO productos (categoria,marca,sku,titulo,tipoDivisa,precio,descuento,existencias)
				VALUES ( "'.trim($value[0]).'" , "'.trim($value[1]).'" , "'.trim($value[2]).'" , "'.trim($value[3]).'" , "'.trim($value[4]).'" , "'.trim($value[5]).'" , "'.trim($value[6]).'" , "'.trim($value[7]).'");';
				if ($insertar = $CONEXION->query($sql)) {
