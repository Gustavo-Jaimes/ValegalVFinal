...
// CAMPOS DE BÚSQUEDA 
	...
	// 		<div hidden><label class="pointer"><i uk-icon="search"></i> Material<br><input type="text" class="uk-input search" data-campo="material"></label></div>
	// 		<div hidden><label class="pointer"><i uk-icon="search"></i> Precio<br><input type="text" class="uk-input search" data-campo="precio"></label></div>
	// 		<div hidden><label class="pointer"><i uk-icon="search"></i> Descuento<br><input type="text" class="uk-input search" data-campo="descuento"></label></div>
	// 	</div>
	// </div>';
...
// TABLA DE PRODUCTOS 
	...
	<div class="uk-width-1-1 margin-v-50">
		<table class="uk-table uk-table-striped uk-table-hover uk-table-small uk-table-middle uk-table-responsive">
			...
			$CONSULTA = $CONEXION -> query("SELECT * FROM $modulo ORDER BY orden LIMIT $prodInicial,$prodspagina");
			while ($rowCONSULTA = $CONSULTA -> fetch_assoc()) {
				...
				$picROW = "";
				$CONSULTA1 = $CONEXION -> query("SELECT * FROM $modulopic WHERE producto = $prodID ORDER BY orden LIMIT 1");
				if ($CONSULTA1->num_rows > 0) {
					$rowCONSULTA1 = $CONSULTA1 -> fetch_assoc();
					$picId=$rowCONSULTA1['imagen'];
					$picROW='';
					$pic=$rutaFinal.$picId;
					if(file_exists($pic)){
						$picROW='
			...
			<!--	<td class="uk-text-nowrap">
					...
					<td>
						<input hidden type="text" class="editarajax uk-input input-number uk-form-small uk-text-right '.$clasePrecio.'" data-tabla="'.$modulo.'" data-campo="precio" data-id="'.$prodID.'" value="'.$rowCONSULTA['precio'].'" tabindex="8">
					</td>
					<td>
						<input hidden type="text" class="editarajax uk-input input-number uk-form-small uk-text-right descuento '.$claseDescuento.'" data-tabla="'.$modulo.'" data-campo="descuento" data-id="'.$prodID.'" value="'.$rowCONSULTA['descuento'].'" tabindex="9">
					</td>
