...
// BOTONES SUPERIORES
	...
	<div class="uk-width-1-1@m">
			<div class="">
				<h3>Imagen inicio</h3>
				<p class="uk-text-muted">Tamaño recomendado 950 px X 720 px. Archivos tipo: JPG </p>
			</div>
		...
		$picmain='../img/contenido/productos/'.$row_CATEGORIAS['imagen'];
		if(strlen($row_CATEGORIAS['imagen'])>0 AND file_exists($picmain)){
			
			echo'
			<div class="uk-panel uk-text-center">
				<a href="'.$picmain.'" target="_blank">
					<img src="'.$picmain.'">
				</a><br><br>
				<button class="uk-button uk-button-danger uk-button-large borrarpicredes"><i uk-icon="icon:trash"></i> Eliminar</button>
			</div>';
		}
		else{
			echo'
		...
		$(document).ready(function() {
			$("#fileuploadermain").uploadFile({
				...
				onSuccess:function(data){ 
					window.location = (\'index.php?rand='.rand(1,1000).'&modulo='.$modulo.'&archivo='.$archivo.'&cat='.$cat.'&id='.$id.'&position=cat&filename=\'+data);
				}
			});
		});
		';
