	...
	echo '
		<div class="uk-width-1-1@s margin-v-20">
			...
			<!--<div class="uk-margin-top">
				...
				<div>
					<span class="uk-text-capitalize uk-text-muted">Nombre:</span>
			...
			<!--	<div>
					<span class="uk-text-capitalize uk-text-muted">Ciudad:</span>
					'.$rowConsultaItem['ciudad'].'
				</div> -->
				<div class="uk-margin bordered uk-text-muted">Descripcion corta:
					'.$rowConsultaItem['txt'].'
				</div>
				<div class="uk-margin bordered uk-text-muted">Descripcion:
					'.$rowConsultaItem['objetivo'].'
				</div>
				<div class="uk-margin bordered uk-text-muted">Fecha:
					'.$rowConsultaItem['fecha'].'
				</div>
			<!--<div class="uk-margin bordered uk-text-muted">
...
// FOTOGRAFÍAS
	echo '
		...
		while ($row_consultaPIC = $consultaPIC -> fetch_assoc()) {
			...
			$picImg =$row_consultaPIC['imagen'];
			$pic=$rutaFinal.$picImg;
			if(file_exists($pic)){
				echo '
					<div id="'.$picId.'">
...
<div class="uk-width-1-1 uk-text-center">
		...
		$consultaPIC = $CONEXION -> query("SELECT * FROM $modulo WHERE id = $id ORDER BY orden,id");
		while ($row_consultaPIC = $consultaPIC -> fetch_assoc()) {
			$picId=$row_consultaPIC['id'];
			$picImg =$row_consultaPIC['pdf'];
			$pic=$rutaFinal.$picImg;
			if(file_exists($pic) && $picImg != ""){
				echo '
					<div id="'.$picId.'">
						<div class="uk-card uk-card-default uk-card-body uk-text-center">
							<a href="'.$pic.'" class="uk-icon-button uk-button-white" target="_blank" uk-icon="icon:image"></a> &nbsp;
							...
							<br>
							<br>
							<a href="'.$pic.'" class="uk-icon-button uk-button-white" target="_blank"><i class="fas fa-file-pdf" style="font-size: 5em"></i></a>;
						</div>
					</div>';
			}
		}
...
$scripts='
	...
	$(document).ready(function() {
		$("#fileuploader").uploadFile({
			url:"'.$ruta.'library/upload-file/php/upload.php",
			fileName:"myfile",
			maxFileCount:1,
			showDelete: \'false\',
			...
			onSuccess:function(data){ 
				window.location = (\'index.php?rand='.rand(1,1000).'&modulo='.$modulo.'&archivo=detalle&id='.$id.'&imagen=\'+data);
			}
		});
	});
	...
	$(document).ready(function() {
		$("#pdfuploader").uploadFile({
			...
			returnType:\'json\',
			onSuccess:function(data){ 
				window.location = (\'index.php?rand='.rand(1,1000).'&modulo='.$modulo.'&archivo=detalle&id='.$id.'&pdf=\'+data);
			}
		});
	});
