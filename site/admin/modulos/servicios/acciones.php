...
//%%%%%%%%%%%%%%%%%%%%%%%%%%    Nuevo galería    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	if(isset($_POST['nuevocatalogo']) AND isset($_POST['titulo']) AND isset($_POST['descripcion'])){
		...
		$curso = trim(htmlentities($_POST['curso']));
		$curso_en = trim(htmlentities($_POST['curso_en']));

		$sql = "INSERT INTO $modulo (titulo, titulo_en, descripcion, descripcion_en, objetivo, objetivo_en, proceso, proceso_en, curso, curso_en) VALUES ('$titulo', '$titulo_en', '$desc', '$desc_en', '$objetivo', '$objetivo_en', '$proceso', '$proceso_en', '$curso','$curso_en')";
...
//    Artículo Editar                   
	if(isset($_REQUEST['editar']) OR isset($editarNuevo)) {
		...
		foreach ($_POST as $key => $value) {
			if ($key=='txt' || $key == 'objetivo' || $key == 'objetivo_en' || $key == 'descripcion' || $key == 'descripcion_en' || $key == 'proceso' || $key=='proceso_en' || $key=='curso' || $key=='curso_en') {
