...
// *****************************
	//	Nuevo Artículo      
		if(isset($_POST['nuevo']) && isset($_POST['pregunta'])){ 
			...
			if (isset($_POST['pregunta_en']))	{ $pregunta_en=htmlentities($_POST['pregunta_en'], ENT_QUOTES);	}else{	$pregunta_en=''; }
			if (isset($_POST['respuesta_en']))	{ $respuesta_en=str_replace("'", "&#039;", $_POST['respuesta_en']);	}else{	$respuesta_en=''; }
			...
			if($pregunta!=""){
				...
				$sql = "INSERT INTO faq (pregunta,respuesta,pregunta_en,respuesta_en,orden)".
					"VALUES ('$pregunta','$respuesta','$pregunta_en','$respuesta_en','99')";
...
// *****************************
	...
	//%%%%%%%%%%%%%%%%%%%%%%%%%%    Borrar Imagen      	 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		if(isset($_POST['borrarslider'])){
			include '../../../includes/connection.php';
			$id=$_POST['id'];
			$consulta = $CONEXION -> query("SELECT * FROM carousel WHERE id= $id");
	...
	//%%%%%%%%%%%%%%%%%%%%%%%%%%    Subir Imagen     	 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		if(isset($_GET['imagenslider'])){
			...
			if(file_exists($rutaInicial.$imagenName)){
				...
				$rutaFinal='../img/contenido/carousel/';
				$imgFinal=rand(111111111,999999999).'.'.$ext;
				if(file_exists($rutaFinal.$imgFinal)){
					$imgFinal=rand(111111111,999999999).'.'.$ext;
				}

				$sql = "INSERT INTO carousel (orden,imagen) VALUES (99,'$imgFinal')";
	...
	// 	COLORES Y TALLAS 
		...
		//  TALLA NUEVA 
			if(isset($_GET['nuevatalla'])){ 
				...
				if (isset($_GET['fechainicio'])) { $fechainicio=htmlentities($_GET['fechainicio'], ENT_QUOTES);   }else{	$tipo=false; $fallo=1; }
				if (isset($_GET['fechafinal'])) { $fechafinal=htmlentities($_GET['fechafinal'], ENT_QUOTES);   }else{	$tipo=false; $fallo=1; }

				// Actualizamos la base de datos
				if($txt!=""){
					$sql = "INSERT INTO productostalla (tipo,txt,fechainicio,fechafinal) VALUES ('$tipo','$txt','$fechainicio','$fechafinal')";
					if($insertar = $CONEXION->query($sql)){
						$cat = $CONEXION->insert_id;
