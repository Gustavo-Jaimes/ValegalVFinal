...
//    Contacto                           
	if (isset($_POST['contacto'])) {
		...
		if(isset($_POST['empresa']) and $_POST['empresa']!='' ){ 	$empresa=htmlentities($_POST['empresa']); }else{ $empresa=false; $fallo=1; $msjTxt.='<br>Falta empresa'; }
		if(isset($_POST['ciudad']) 	and $_POST['ciudad']!='' ){ 	$ciudad=htmlentities($_POST['ciudad']); }else{ $ciudad=false; $fallo=1; $msjTxt.='<br>Falta ciudad2'; }
		...
		if(isset($_POST['mensaje']) and $_POST['mensaje']!='' ){ 	$mensaje=htmlentities($_POST['mensaje']); }else{ $mensaje=false; $fallo=1; $msjTxt.='<br>Falta mensaje'; }
		...
		switch ($enviarcorreo) {
			case 1:
				...
				$cuerpoCorreo = '
					...
					<a href="'.$row_CONSULTA['linkguia'].'" style="background-color:'.$mailButton.';font-weight:700;border-radius:8px;padding-left:30px;padding-right:30px;padding-top:10px;padding-bottom:10px;color:white;text-decoration:none;">Rastrear env&iacute;o</a><br><br><br><br>
...
//    Obtener talla y color              
	if (isset($_POST['tallaycolor'])) {
		...
		if ($numItems>0) {
			...
			$msjClase='success';
			$msjTxt=$color.'-'.$talla;
			$xtras='Fecha seleccionada: '.$talla.'<br>'.$color;
		}
	}
...
//    Cambiar foto de perfil             
	if(isset($_GET['profilepicchange'])){
		...
		if ($fallo==0) {
			...
			$row_CONSULTA = $CONSULTA -> fetch_assoc();
			$imagenSQL=$rutaFinal.$row_CONSULTA['imagen'];
			...
			// Actualizar la base de datos
			if($actualizar = $CONEXION->query("UPDATE usuarios SET imagen = '$imgFinal' WHERE id = $uid")){
				$estatus='?estatus=Ok';
			}else{
